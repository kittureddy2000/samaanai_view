...
export function AuthProvider({ children }) {
  ...
  const clearTokens = () => {
    ...
    // Clear finance API client headers if it exists
    if (window.financeApiClient) {
      delete window.financeApiClient.defaults.headers.common['Authorization'];
    }
  };
  ...
  const setAuthTokens = (access, refresh) => {
    ...
    // Update finance API client headers if it exists
    if (window.financeApiClient) {
      window.financeApiClient.defaults.headers.common['Authorization'] = `Bearer ${access}`;
    }
