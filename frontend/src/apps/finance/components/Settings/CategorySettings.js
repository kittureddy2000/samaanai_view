...
const CategorySettings = () => {
  ...
  const loadCategories = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get('/finance/spending-categories/');
  ...
  const handleDeleteCategory = async (categoryId) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await apiClient.delete(`/finance/spending-categories/${categoryId}/`);
  ...
  const handleSaveCategory = async () => {
    try {
      if (editingCategory) {
        ...
        const response = await apiClient.patch(`/finance/spending-categories/${editingCategory.id}/`, newCategory);
        setCategories(prev => prev.map(cat => 
          cat.id === editingCategory.id ? response.data : cat
        ));
      } else {
        // Create new category
        const response = await apiClient.post('/finance/spending-categories/', newCategory);
