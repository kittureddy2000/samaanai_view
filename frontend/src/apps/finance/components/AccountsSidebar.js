...
const AccountsSidebar = ({ accounts, loading, selectedAccount, onSelect, onAccountAdded, onClose, onRefresh }) => {
  ...
  const getAccountStatus = (account) => {
    ...
    if (account.type?.toLowerCase() === 'credit') {
      ...
      if (utilization > 0.9) return { icon: <ErrorIcon />, color: '#ef4444' };
      if (utilization > 0.7) return { icon: <WarningIcon />, color: '#f59e0b' };
      return { icon: <CheckCircleIcon />, color: '#10b981' };
    }
    
    // Banking account warnings
    if (balance < 100) return { icon: <WarningIcon />, color: '#f59e0b' };
    if (balance < 0) return { icon: <ErrorIcon />, color: '#ef4444' };
...
const NetWorthValue = styled.div`
  font-size: 1.75rem;
  font-weight: 700;
  color: ${props => props.$isNegative ? '#dc2626' : '#1e293b'};
  line-height: 1.2;
`;
...
const AccountItemContainer = styled.div`
  ...
  border-left: ${props => props.$isSelected ? '3px solid #2563eb' : '3px solid transparent'};
...
const AccountBalance = styled.div`
  font-weight: 600;
  font-size: 0.875rem;
  color: ${props => props.$isNegative ? '#dc2626' : '#111827'};
  text-align: right;
  white-space: nowrap;
`;
