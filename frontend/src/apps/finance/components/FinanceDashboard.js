...
import { getInstitutions, getAccounts, getSpendingCategories, getDashboardData, financeApiClient } from '../services/api';
...
const FinanceDashboard = ({ accounts, loading }) => {
  ...
  return (
    <DashboardContainer>
      ...
      <Paper elevation={2} sx={{ padding: '16px', marginBottom: '24px' }}>
      ...
      <TransactionCard style={{ marginTop: '24px' }}>
        ...
        {/* Enhanced Filter Row */}
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', marginBottom: '20px', padding: '16px', backgroundColor: '#f8fafc', borderRadius: '12px' }}>
          <TextField 
          ...
          <Box sx={{ marginLeft: 'auto', display: 'flex', gap: 1 }}>
            <Button onClick={exportTransactionsToCSV} startIcon={<FileDownloadIcon />} size="small" variant="outlined">
              CSV
            </Button>
            <Button onClick={exportTransactionsToExcel} startIcon={<FileDownloadIcon />} size="small" variant="outlined">
              Excel
            </Button>
                </Box>
            </Box>
